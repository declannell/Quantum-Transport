...
class Interacting_GF:
    ...
    def __init__(self, _kx: float, _ky: float, _voltage_step: int, _self_energy_many_body):
        ...
        self.interacting_gf = [create_matrix(parameters.chain_length) for r in range(parameters.steps)]       
    ...
    def get_interacting_gf(self):
        ...
        for r in range(0, parameters.steps):
            for i in range(0, parameters.chain_length):
                for j in range(0, parameters.chain_length):
                    if (i == j):
                        inverse_green_function[i][j] = parameters.energy[r].real - self.effective_hamiltonian[r][i][j] - self.self_energy_many_body[r][i]
                    else:
                        inverse_green_function[i][j] = - self.effective_hamiltonian[r][i][j]

            self.interacting_gf[r] = la.inv(inverse_green_function, overwrite_a=False, check_finite=True)

    def plot_greenfunction(self):
        for i in range(0, parameters.chain_length):

            plt.plot(parameters.energy, [e[i][i].real for e in self.interacting_gf], color='red', label='Real Green up') 
            plt.plot(parameters.energy, [e[i][i].imag for e in self.interacting_gf], color='blue', label='Imaginary Green function')
            j = i +1
...
def impurity_solver(gf_local_up: List[complex], gf_local_down: List[complex]):

    self_energy_up = [0 for z in range(0, parameters.steps)]  
    self_energy_down = [0 for z in range(0, parameters.steps)] 


    return self_energy_up, self_energy_down
    """
...
def dmft(voltage: int, kx: List[float], ky: List[float]):
    self_energy_mb_up = [[0 for i in range(0, parameters.steps)]for z in range(0, parameters.steps)]
    self_energy_mb_down = [[0 for i in range(0, parameters.steps)]for z in range(0, parameters.steps)]
    ...
    while ( difference > 0.0001 and count < 15) : #these allows us to determine self consistency in the retarded green function
        count += 1
        gf_interacting_up = [[Interacting_GF(kx[i], ky[j], voltage, self_energy_mb_up) for i in range(0, parameters.chain_length_x)] for j in range(0, parameters.chain_length_y)]
        gf_interacting_down = [[Interacting_GF(kx[i], ky[j], voltage, self_energy_mb_down) for i in range(0, parameters.chain_length_x)] for j in range(0, parameters.chain_length_y)]

        #this quantity is the green function which is averaged over all k points.
        gf_local_up = [create_matrix(parameters.chain_length) for z in range(0, parameters.steps)]
        gf_local_down = [create_matrix(parameters.chain_length) for z in range(0, parameters.steps)]
    ...
    #for r, i, j in cartesian(parameters.steps, parameters.chain_length, parameters.chain_length):
        ...
        for r in range(0, parameters.steps):
            for i in range(0, parameters.chain_length):
                for j in range(0, parameters.chain_length):
                    (up, down) = sum_gf_interacting(r, i, j, gf_interacting_up, gf_interacting_down)
        ...
        for r in range(0 , parameters.steps):
            for i in range(0 , parameters.chain_length):
                for j in range(0 , parameters.chain_length): #this is due to the spin_up_occup being of length chain_length
                    differencelist[r + i + j] = abs(gf_local_up[r][i][j].real - old_green_function[r][i][j].real)
                    differencelist[n + r + i + j] = abs(gf_local_up[r][i][j].imag - old_green_function[r][i][j].imag)
        ...
        if( parameters.interaction_order != 0):
            for i in range(0, parameters.chain_length):
                impurity_self_energy_up, impurity_self_energy_down = (
    ...
    for i in range(0, parameters.chain_length):
            ...
            plt.plot(parameters.energy, [e[i][i].real for e in gf_local_up], color='red', label='Real Green up') 
            plt.plot(parameters.energy, [e[i][i].imag for e in gf_local_up], color='blue', label='Imaginary Green function')
            j = i +1
    ...
    for i in range( 0, parameters.chain_length ):
        ...
        plt.plot(parameters.energy , [e[i].imag for e in self_energy_mb_up], color='blue', label='imaginary self energy' ) 
        plt.plot(parameters.energy , [e[i].real for e in self_energy_mb_up] , color='red' , label='real self energy') 
...
def main():
    ...
    green_function_up, green_function_down = dmft(parameters.voltage_step, kx, ky)
